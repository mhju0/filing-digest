...
│  GET  /companies/{id}/digest      POST /chat    POST /ingest  │
│                                                               │
│  ┌─────────────┐   ┌──────────────┐   ┌───────────────────┐  │
│  │ Stub Store   │   │ Ingest (stub) │   │ Digest / Chat     │  │
│  │ (in-memory)  │   │ job queue     │   │ (deterministic)   │  │
│  └─────────────┘   └──────────────┘   └───────────────────┘  │
└───────┬──────────────────────┬────────────────────────────────┘
...
- **v0.1(현재)**: 외부 API 실호출 없음. 삼성전자·Apple 두 회사의 스텁(stub) 데이터로
  전 엔드포인트가 결정적(deterministic)으로 동작한다.
- **Phase 2**: DART/SEC 실연동, 파싱/청킹, 임베딩 + RAG, LLM 요약 파이프라인.
...
│   ├── app/                 #   라우터, 스키마(pydantic), 설정, 스텁 데이터
...
1. **수치(`MetricCard.value`)는 DART/SEC 구조화 데이터에서만 온다.**
   LLM이 숫자를 생성·추정·보정하는 일은 없다. v0.1의 스텁 숫자조차도
   `citation_id`로 스텁 `Citation`에 연결되어 이 파이프라인 규약을 강제한다.
2. **LLM은 요약/서술(narrative)만 담당한다.**
   digest의 `summary_ko`/`summary_en`, chat의 `answer`가 LLM의 영역이며(v0.1은 결정적 스텁),
   여기에도 숫자를 "지어내는" 것은 금지된다. 서술에 등장하는 수치는 구조화 데이터를 그대로 인용한다.
...
POST /chat -> 200
  request  ChatRequest  = {"company_id": str|null, "question": str(1자 이상), "language": "ko"|"en" (default "ko")}
  response ChatResponse = {"answer": str, "language": "ko"|"en", "citations": [Citation]}
