...
| `net_income` | `ifrs-full_ProfitLoss` | 당기순이익(손실) | IS | 15,487,100,000,000 | **총** 당기순이익. IS/CIS/CF **3곳에 동일값 중복** → sj_div=IS 하나만 채택 |
| `net_income_attributable` | `ifrs-full_ProfitLossAttributableToOwnersOfParent` | 지배기업 소유주지분 순이익 | IS | 14,473,401,000,000 | **지배주주귀속** (총 − 비지배지분). IS/CIS 중복 → sj_div=IS 하나만 채택 **[Verified]** |
| `eps` | `ifrs-full_BasicEarningsLossPerShare` | 기본주당이익 | IS | 2,131 | **주당 원(per-share)** — 절대금액 아님. **Decimal 파싱** |
| `eps_diluted` | `ifrs-full_DilutedEarningsLossPerShare` | 희석주당이익 | IS | 2,131 | 희석 EPS. per-share, **Decimal 파싱** **[Verified]** |
...
- **net_income 총/지배주주 둘 다 저장** **[Verified]**: `ifrs-full_ProfitLoss`(총, 15,487,100,000,000)와
  `ifrs-full_ProfitLossAttributableToOwnersOfParent`(지배주주귀속, 14,473,401,000,000)를 각각
  `net_income` / `net_income_attributable`로 **모두** 적재한다(총 > 지배주주, 차이=비지배지분).
  둘 다 IS/CIS(총은 +CF)에 중복되므로 **account_id별로** sj_div=IS 하나만 남긴다(dedup은 account_id 단위 그룹핑).
- **EPS 소수점** **[Verified]**: 주당이익은 **정수로도 소수로도** 올 수 있다(삼성 2023은 정수 `2131`이지만
  회사·기간에 따라 소수). 반올림 오차 방지를 위해 정수 KRW 금액 파서(int)와 별도로 **`Decimal` 파서**로
  파싱한다. `eps`(기본)와 `eps_diluted`(희석) 둘 다 저장. `float`는 쓰지 않음.
...
### 비정상 마크업 함정 (★) **[Verified]** — `decode_dart_bytes`/`extract_dsd_prose` 실측

- DSD(A) 본문은 **엄밀한 well-formed XML 이 아니다**. 산문(`<P>`) 안에 **이스케이프 안 된
  리터럴 `&`·`<`** 가 그대로 들어있어 expat(=defusedxml) 이 `not well-formed (invalid token)` 으로
  거부한다. 삼성 2023 사업보고서(`20240312000736`) 본문 실측: **bare `&` 529건**(예: `R&D`),
  **bare `<` 8건**(예: `< TV 시장점유율 추이 >`). **[Verified]**
- 대응: 파서에 넘기기 전 **보수적으로만** 이스케이프한다(진짜 태그 구조는 건드리지 않음):
  - 유효 엔티티(`&amp;`/`&#..;` 등)로 시작하지 않는 `&` → `&amp;`
  - 태그/주석/PI(뒤가 name·`/`·`!`·`?`)로 시작하지 않는 `<` → `&lt;`
  - 이 2개만으로 6.15MB 본문이 defusedxml 로 전부 파싱되며, 복원된 태그 카운트가 아래 빈도와 일치
    (`SECTION*`=53, `TITLE`=135, `P`=17065). **[Verified]**
- `<?xml ... encoding="euc-kr"?>` 선언이 남아있으면 ElementTree 가 **str 입력을 거부**
  ("Unicode strings with encoding declaration are not supported") → 디코드 후 **선언 제거**하고 str 로 파싱.
- 인라인 서술 숫자(예: `679,267백만원`)는 **표가 아니라 산문**이므로 제외 대상이 아니다.
  제외 대상은 `<TABLE>` 요소뿐(수치표는 §3 재무 API 가 단일 출처).
...
- **[구현 완료]** `DartClient` httpx 구현: `corpCode.xml`(ZIP/XML) · `list.json` ·
  `fnlttSinglAcntAll.json` · `document.xml`(fetch→ZIP 멤버 선별→bytes). ZIP 시그니처 분기,
  status 코드 처리 완료. (레이트리밋 백오프는 미구현 **[TODO]**.)
- **[구현 완료]** DSD 원문 파싱: `decode_dart_bytes`(UTF-8 우선/cp949 폴백) →
  `detect_document_format`(dsd/xforms/unknown) → `extract_dsd_prose`(SECTION 단위 산문,
  `<TABLE>` 전량 제외). 청킹(→`filing_chunks`)·임베딩·DB write 는 다음 스텝(④-c).
- **[구현 예정 Phase 2]** xforms(B) HTML 파서는 **감지만** 되어 있고 본문 추출 미구현 **[TODO]**.
  6MB 본문은 통째로 파싱(스트리밍 최적화 **[TODO]**).
...
- **수동 확인 필요 항목**:
  ...
  - EPS(`ifrs-full_BasicEarningsLossPerShare`/`...Diluted...`): 소수점 허용·`Decimal` 파싱으로
    확정(§3 **[Verified]**). 다만 **분기보고서에서 누적/단분기 구분**은 여전히 **[Unknown]**.
